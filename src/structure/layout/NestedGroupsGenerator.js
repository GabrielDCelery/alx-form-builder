...
class NestedGroupsGenerator {
    ...
    build (_groupConfigs = []) {
        if (_groupConfigs.length === 0) {
            _groupConfigs = _.cloneDeep(DEFAULT_GROUP_CONFIG);

            this.$(`[id^="${this.PREFIX_FIELD_ID}"]`).map((_index, _elem) => {
                const _fieldId = this.$(_elem).attr('id');

                if (_fieldId === this.ALX_ID_API_KEY_FIELD) {
                    return;
                }

                _groupConfigs[0].fields.push(_fieldId);
            });
        }
